...
function define_chars()
  ...
  for i, c_attrs in ipairs(c_attr_list) do
    ...
    chars[i].finishing_mvs = define_acs(fa_attr_list[i], "name,distance,fps,dmg_sp,sps,sequence,fmr1_fps,fmr1_sps,fmr2_fps,fmr2_sps")
  end
...
function define_pl(id, p)
  ...
  local p = string_to_hash("ac_stack,ac_stack_timeout,char,ca,caf,cap,facing,has_locked_controls,has_joined,highlighted_char,hp,held_buttons,held_buttons_timer,id,input_detection_delay,is_x_shifted,is_y_shifted,next_cbs,particle_sets,previous_buttons,previous_directionals,pj,released_buttons,st_timers,t,x,y", { "", ac_stack_timeout, p.char, acs.idle, 0, {}, is_p1 and forward or backward, false, p.has_joined, is_p1 and 1 or 4, 100, nil, 0, id, 0, false, false, next_cbs, {}, nil, nil, nil, nil, { frozen = 0, invisible = 0, morphed = 0 }, 0, is_p1 and 36 or 127 - 36 - sp_w, y_bottom_limit })
